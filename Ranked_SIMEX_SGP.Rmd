...
Before proceeding, it is useful to review the process used to compute the SIMEX measurement error (ME) adjusted SGPs.  This methodological review is meant to outline the process in general and highlight the areas in which additional efficiency might be gained.  This section is followed by a discussion of the implementation of the Ranked SIMEX system.  For a detailed review of the SIMEX measurement error correction method applied to SGP analyses, see Shang, VanIwaarden and Betebenner [-@ShangVanIBet:2015], and for an in-depth treatment of SIMEX in general see Carroll, Ruppert, Stefanski, & Crainiceanu [-@Carroll:2006].
...
```{r, cache=TRUE, echo=FALSE, include=FALSE, MarginalDists}
  ...
  pdf(file="Math_G8_Marginal_Distributions-1_Prior.pdf", width=12, height=4)
  par(mfrow=c(1,3))
  ...
  pdf(file="Math_G8_Marginal_Distributions-2_Prior.pdf", width=8, height=2.67)
  par(mfrow=c(1,3))
  tmp.label <- "Percentile"
...
```{r, results="asis", echo=FALSE, G8_Math_MargDists}
		placeFigure(page.break= FALSE,
			files = "./img/Math_G8_Marginal_Distributions-1_Prior.pdf",
...
```{r, cache=TRUE, echo=FALSE, include=FALSE, GOFplots}
  ...
  dat <- Agg_Data_Long[SGP_NORM_GROUP %in% c("2015/MATHEMATICS_6; 2016/MATHEMATICS_7; 2017/MATHEMATICS_8", "2016/MATHEMATICS_7; 2017/MATHEMATICS_8"),
	...
	gofSGP(dat, years='2017', content_areas="MATHEMATICS", use.sgp="SGP_SIMEX_ORDER_1", output.format="PNG") # Run before SIMEX and Rename folder???
	gofSGP(dat, years='2017', content_areas="MATHEMATICS", use.sgp="SGP_SIMEX_RANKED_ORDER_1", output.format="PNG") # Run before SIMEX and Rename folder???
...
```{r, results='asis', echo=FALSE, Student_EOG}
	student.cor.grd <- Agg_Data_Long[YEAR=='2017' & VALID_CASE=='VALID_CASE'][, list(
		`$\\\rr_ {  Test Scores}$` = round(cor(SCALE_SCORE, SCALE_SCORE_PRIOR_STANDARDIZED, use='pairwise.complete'), 3),
		`$\\\rr_ {  SGP}$` = format(round(cor(SGP, SCALE_SCORE_PRIOR_STANDARDIZED, use='pairwise.complete'), 3), nsmall = 3),
		`$\\\rr_ {  SIMEX SGP}$` = round(cor(SGP_SIMEX, SCALE_SCORE_PRIOR_STANDARDIZED, use='pairwise.complete'), 3),
		`$\\\rr_ {  Ranked SIMEX}$` = round(cor(SGP_SIMEX_RANKED, SCALE_SCORE_PRIOR_STANDARDIZED, use='pairwise.complete'), 3)),
		# N_Size = sum(!is.na(SGP))), #  Leave off to help anonymize the State
...
list(Mean_SGP=mean(SGP, na.rm=T),
     Mean_SIMEX=mean(SGP_SIMEX, na.rm=T),
     Mean_Ranked_SIMEX=mean(SGP_SIMEX, na.rm=T),
     Prior_SS=mean(SCALE_SCORE_PRIOR_STANDARDIZED, na.rm=T), N=.N),
	keyby=c("SCHOOL_NUMBER", "CONTENT_AREA", "GRADE", "YEAR")]


cor.no.grd <- smry[!is.na(Mean_SGP) & N > 14][,
	list(R_Mean_SGP = round(cor(Mean_SGP, Prior_SS, use="complete"), 3),
       R_Mean_SIMEX = round(cor(Mean_SIMEX, Prior_SS, use="complete"), 3),
       R_Mean_Ranked_SIMEX = round(cor(Mean_Ranked_SIMEX, Prior_SS, use="complete"), 3),
			 N=.N),
...
```{r, results='asis', echo=FALSE, School_EOG_Correlations}
	tmp_tbl <- cor.no.grd
	tmp.cap <- "2014 to 2017 School Level EOG Correlations between Mean Prior Standardized Scale Score and Aggregate SGPs by Content Area."
	...
	cat(dualTable(as.matrix(tmp_tbl), align=c('r', 'r', rep('c', dim(tmp_tbl)[2]-2)), caption = tmp.cap, n.rgroup=rep(4,2)))
...
```{r, results='asis', echo=FALSE, School_Grade_EOG_Correlations}
	...
	tmp.cap <- "2017 School Level EOG Correlations between Mean Prior Standardized Scale Score and Aggregate SGPs by Content Area and Grade."
